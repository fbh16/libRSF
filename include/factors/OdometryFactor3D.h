...
namespace libRSF
{
  ...
  class OdometryFactor3D4DOF_ECEF : public BaseFactor<ErrorType, true, true, 3, 1, 3, 1>
  {
   public:
    ...
    template <typename T>
    VectorT<T, 4> Evaluate(const T * const Pos1, const T * const Yaw1, const T * const Pos2, const T * const Yaw2) const
    {
    ...
    {
      return this->Error_.template weight<T>(this->Evaluate(Pos1, Yaw1, Pos2, Yaw2),
                                             Params...);
    }
  ...
  class OdometryFactor3D6DOF : public BaseFactor<ErrorType, true, true, 3, 4, 3, 4>
  {
   public:
    ...
    /** geometric error model */
    template <typename T>
    VectorT<T, 6> Evaluate(const T * const Pos1, const T * const Quat1, const T * const Pos2, const T * const Quat2) const
    {
      const Vector6 Odometry = this->MeasurementVector_;
    ...
    /** combine probabilistic and geometric model */
    template <typename T, typename... ParamsType>
    bool operator()(const T * const Pos1, const T * const Quat1, const T * const Pos2, const T * const Quat2, ParamsType... Params) const
    {
      return this->Error_.template weight<T>(this->Evaluate(Pos1, Quat1, Pos2, Quat2),
                                             Params...);
    }
    ...
    void predict(const std::vector<double *> &StatePointers) const
    {
      OdometryModel6DOF<double>::applyForward(StatePointers[0],
                                              ...
                                              StatePointers[2],
                                              StatePointers[3],
                                              this->MeasurementVector_.head(3), // velocity
                                              this->MeasurementVector_.tail(3), // turnrate
                                              this->DeltaTime_);
    //   std::cout << "head: " << this->MeasurementVector_.head(3) << std::endl;
    //   std::cout << "tail: " << this->MeasurementVector_.tail(3) << std::endl;
    }
  };

  template <typename ErrorType>
  struct FactorTypeTranslator<FactorType::Odom4, ErrorType>
  {
    using Type = OdometryFactor3D4DOF<ErrorType>;
  };
  template <typename ErrorType>
  struct FactorTypeTranslator<FactorType::Odom4_ECEF, ErrorType>
  {
    using Type = OdometryFactor3D4DOF_ECEF<ErrorType>;
  };
  template <typename ErrorType>
  struct FactorTypeTranslator<FactorType::Odom6, ErrorType>
  {
    using Type = OdometryFactor3D6DOF<ErrorType>;
  };
}  // namespace libRSF
