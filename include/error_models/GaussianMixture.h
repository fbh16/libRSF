...
namespace libRSF
{
  ...
  class GaussianMixture
  {
   public:
      ...
      while ((!Converged && !ReachedMaxIteration) || Merged || Pruned)
      {
        ...
        switch (Config.EstimationAlgorithm)
        {
          case ErrorModelTuningType::EM:
          {
            ...
            if (M > 1)
            {
              // GMM E-Step: expectation
              LikelihoodSum = this->computeProbability(DataMatrix, Probability, myProbability);  // 滑动窗口中每个状态n属于GMM第m组成分的概率
            ...
            {
              // myProbability.row(0) = Mixture_.at(0).computeNegLogLikelihood(DataMatrix, CurrentFactorType);
              myProbability.row(0) = Mixture_.at(0).computeLikelihood(DataMatrix);
              LikelihoodSum = myProbability.sum();
            }
          }
          break;
        ...
        if (std::isnan(LikelihoodSum) || std::isnan(LikelihoodSumOld) || 
            std::isinf(LikelihoodSum) || std::isinf(LikelihoodSumOld)) 
        {
            std::cerr << "Invalid input: LikelihoodSum = " << LikelihoodSum 
                      << ", LikelihoodSumOld = " << LikelihoodSumOld << std::endl;
            LikelihoodChange = 0.0;
        }
        if (std::abs(LikelihoodSum) < epsilon) 
        {
            LikelihoodChange = 0.0; // 或者其他默认值，视情况而定
        } 
        else {
            LikelihoodChange = std::abs(LikelihoodSum - LikelihoodSumOld) / std::abs(LikelihoodSum);
        }

        if (k > 1 && LikelihoodChange < Config.MinLikelihoodChange)
        {
          Converged = true;
        //   std::cout << "EM Alogorithm is Converged." << std::endl;
        }
        else
        {
          Converged = false;
        ...
        //   std::cout << "EM Alogorithm Reached MaxIteration." << std::endl;
        }
        else
        {
        ...
        std::cout << "EM Iter: " << k 
        << "  LikelihoodSum: " << std::fixed << std::setprecision(10) << LikelihoodSum  
        << "  LikelihoodChange: " << std::fixed << std::setprecision(10) << LikelihoodChange << std::endl; 
    ...
    void mapMinMax(Matrix &Block, double minVal = 0, double maxVal = 1) const
    {
      double minData = Block.minCoeff();
      double maxData = Block.maxCoeff();
      Block = ((Block.array() - minData) / (maxData - minData)) * (maxVal - minVal) + minVal;
    }

    void matrixBlock(Matrix &Probability, std::vector<int> &BlockInterval) const
    {
      std::vector<Matrix> subMatrices;

      Index startCol = 0;
      for (int colNum : BlockInterval)
      {
        Index endCol = colNum;
        subMatrices.push_back(Probability.block(0, startCol, Probability.rows(), endCol - startCol));
        startCol = endCol;
      }

      // for (Matrix& block : subMatrices) {
      //     this->mapMinMax(block);
      // }

      startCol = 0;
      for (auto &block : subMatrices)
      {
        Probability.block(0, startCol, block.rows(), block.cols()) = block;
        startCol += block.cols();
      }
    }

    //* E-step2
    double computeProbability(const ErrorMatType &DataVector, Matrix &Probability, Matrix &myProbability) const
    {
    ...
    //   this->computeNegLogLikelihood(DataVector, Probability);
    //   std::cout << "Probability:\n" << Probability << std::endl;

      const double LikelihoodSum = Probability.sum();  // Eq.5
      
    //   std::cout << "non-normalized prob:\n" << Probability <<std::endl;
    ...
    void computeMixtureParameters(const ErrorMatType &DataVector, const Matrix &Likelihood, const EstimationConfig &Config, const Matrix &myLikelihood)
    {
      const int M = Mixture_.size();
      for (int m = 0; m < M; m++)
      {
    ...
    void printParameter() const
    {
      ...
      for (int n = 0; n < static_cast<int>(Mixture_.size()); ++n)
      {
        // std::cout << Mixture_.at(n).getCovariance() << "\n" << std::endl;
        // std::cout << Mixture_.at(n).getSqrtInformation() << std::endl;
