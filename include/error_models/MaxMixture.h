      ...
      bool weight(const VectorT<T, Dim> &RawError, T* Error) const
      {
        if(this->Enable_)
        {
          ...
          for(int nComponent = 0; nComponent < NumberOfComponents; ++nComponent)
          {
            ...
            // std::cout << "author sqrt{-2 * ln[c_j / max(c_j, 1e-10)]}:\n" 
            //           << sqrt(ceres::fmax(-2.0 * T(log(Mixture_.template getLinearPartOfComponent<T>(nComponent, RawError) / Normalization_)), T(1e-10))) << std::endl;

            // std::cout << "mine sqrt{-2 * ln[c_j / max(c_j, 1e-10)]}:\n" 
