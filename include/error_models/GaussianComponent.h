...
namespace libRSF
{
  ...
  class GaussianComponent
  {
   public:
    ...
    //* estimate process
    /** compute probability of a meas-estim-pair for E step of GMM-EM */
    ...
    //   std::cout << "Mean:\n" << Mean_.array() << std::endl;
    //   std::cout << "Weighted Error:\n" << WeightedError << std::endl;
    ...
    //   std::cout << "Weighted ErrorMatrix:\n" << WeightedError << std::endl;
      /***
       ** 求P(e_i | w, u, I^{1/2})
       ** Scaling_: weight * det(I^{1/2})
       ** return: weight * det(I^{1/2}) * exp{1/2 || I*[(x-z)+u] ||^2}
       */
    //   std::cout << "ErrorMatrix ColSum:\n" << WeightedError.array().square().colwise().sum() << std::endl;
    ...
    {
      const double LikelihoodSum = Likelihoods.sum();//归一化
